...
SORT_BY      = "AVG"                           # default sort column (highest first)
HTML_OUT     = "index.html"
...
.flag{display:inline-block;margin-left:8px;font:600 9.5px/1 ui-sans-serif,sans-serif;
  letter-spacing:.08em;color:var(--flag);border:1px solid var(--flag);
...
<p class="legend">
  ...
  Rows marked <span class="flag">OVERDUE</span> have gone longer since their last hit than their usual gap between hits. &nbsp;
...
document.querySelectorAll('th').forEach((th,ci)=>{
  // first click: player column A->Z (asc), number columns highest-first (desc)
  let asc = ci===1;
  th.addEventListener('click',()=>{
    ...
    rows.sort((a,b)=>{
      ...
      if(xn&&yn) return dir?nx-ny:ny-nx;
      if(xn) return 1; if(yn) return -1;        // blanks/NaN always sort last
...
def write_html(df: pd.DataFrame, path: str):
    ...
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        gap = row["avg_PA_between_hits"]
        since = row["PA_since_last_hit"]
        overdue = (not pd.isna(gap)) and (since > gap)
        ...
        for key, _, kind in COLS:
            ...
            if key == "player" and overdue:
                disp += "<span class='flag'>OVERDUE</span>"
            style = ""
        ...
        tr_class = " class='overdue'" if overdue else ""
        body.append(f"<tr{tr_class}>" + "".join(cells) + "</tr>")
